...
export const searchCustomersSubscriptions = async (
    ...
    try {
        ...
        // Support both shapes: payload is array OR { data: array }
        ...
        // Filter out unsuccessful entries (support boolean or string flags)
        const validEntries = entries.filter(
            (entry: any) =>
                entry && entry.success !== false && entry.success !== 'false'
        )

        // Flatten the nested data arrays, guard against non-arrays
        const flattened = validEntries.flatMap((entry: any) =>
            Array.isArray(entry?.data) ? entry.data : []
        )

        // Ensure unique subscriptions by `id`
        const uniqueSubscriptions = Object.values(
            flattened.reduce((acc: Record<string, any>, subscription: any) => {
                const key = subscription?.id ?? JSON.stringify(subscription)
                if (!acc[key]) acc[key] = subscription
                return acc
            }, {})
        )

        return uniqueSubscriptions as any[]
    } catch (error) {
        console.error('Error searching customer:', error)
        throw error
    }
